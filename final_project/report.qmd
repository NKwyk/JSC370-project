...
The analysis uses `pm25_weather_local_2024_2.0.csv`, a merged monitor-day dataset built from EPA AQS PM2.5 monitor records, EPA AQS weather-related daily summary files, and NOAA Climate Data Online API temperature and precipitation variables. The unit of analysis is one PM2.5 monitor on one day.
...
The outcome for regression is `arithmetic_mean`, the daily mean PM2.5 concentration. The classification outcome is `high_pm25_day`, equal to 1 when daily PM2.5 is greater than 35 ug/m3. The reason I define high_pm25_day as a daily PM2.5 concentration greater than 35 µg/m³, using the U.S. EPA 24-hour PM2.5 standard as a policy-relevant threshold. This threshold is used to identify unusually high daily PM2.5 monitor-days, rather than to make formal regulatory attainment determinations.
...
ax.barh(reg_importance["feature"], reg_importance["importance"], color="#16817a")
...
fig, ax = plt.subplots(figsize=(8, 5))
ax.barh(clf_importance["feature"], clf_importance["importance"], color="#b5453c")
ax.set_xlabel("Share of model importance")
ax.set_ylabel("")
